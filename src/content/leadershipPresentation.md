...
## Board mandate — model agnosticism at the harness level

NovaMind’s board wants **more model-agnostic infrastructure**. The technical answer is not vague “multi-cloud” talk: **`AgentDefinition.model`** is **per-lane configuration**—the **model id is a parameter** on each agent definition, not a monolithic “the whole architecture is vendor X” choice. **Delegation, MCP tools, hooks, permissions, sessions, and structured outputs** define the **workflow shape**; when Anthropic, OpenAI, Google, or another vendor ships a better SKU, you **update the model field and re-run frozen eval**, not **re-architect** the literature / data / hypothesis graph. That is **model agnosticism at the harness level**—what the mandate is actually asking for.
