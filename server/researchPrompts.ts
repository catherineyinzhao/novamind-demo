...
3. **### Per-phase success criteria** — one bullet each for literature, data, and hypothesis phases (what “done well” means for each).
4. End with a fenced JSON code block labeled \`nova_delegate_plan\` with shape:
{"phases":[{"id":"literature","focus":""},{"id":"data","focus":""},{"id":"hypothesis","json":["ranked_hypotheses.json"]}],"citation_policy":"tool_pmids_only","eval_dimensions":["citation_accuracy","structured_output","tool_efficiency"]}
...
You have access to **fetch_experiment_summary** (via MCP) — structured xenograft / endpoint demo metrics.
...
2) Map literature claims to cohort signals: **supported**, **tension**, or **not addressed** by client data; note which fetch supports each judgment when possible.
3) End with **### Data analysis memo** (short bullets) that explicitly compares insights across the two fetches.`
